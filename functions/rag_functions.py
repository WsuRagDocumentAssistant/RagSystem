...
ANSWER_PROVIDERS  = ["gpt", "claude", "gemini"]

#

#────────────────────────────────────────────────
QUERY_CHAIN = ["embed_query_function", "hybrid_search_function","build_context_function", "rerank_function"]
#------------------------------------------------┌> 질의 테스트 생성
tasks["레그 준비"] = ["warmup_function"]
tasks["레그 실행"] = ["parse_function", "chunk_function", "vocab_function","save_vocab_function", "embed_function", "save_document_with_vector_function"]
tasks["레그 청킹"] = ["parse_function", "chunk_function"]   # 모델·DB 없이 확인용
tasks["레그 검색"] = QUERY_CHAIN                             # 맥락까지. LLM 안 씀
tasks["레그 답변"] = QUERY_CHAIN + ["answer_function"]
tasks["레그 병합"] = QUERY_CHAIN + ["answer_function", "merge_function"]
...
def warmup_function(*args, **kwargs):
    """모델·DB 를 미리 올려둔다. 첫 질의가 몇십 초 걸리는 걸 앞으로 당긴다."""
    rag = get_controller()
...
def answer_function(*args, **kwargs):
    ...
    다듬기는 동시에 나간다 — 걸리는 시간이 합이 아니라 가장 느린 하나다.
    """
    query, contexts = args[0]
    rag = get_controller()
...
def merge_function(*args, **kwargs):
    ...
    답변이 하나면 모듈이 호출 없이 그대로 돌려준다.
    """
    answers = args[0]
    merged = get_controller().merge(QUERY, [a["answer"] for a in answers],
                                    provider=MERGE_WITH)
